...
(defcustom electric-sentence-end-after-quote t
  "Whether to add an extra space after quotes."
  :type '(boolean))

(defcustom electric-sentence-end-quotes '(?\" ?”)
  "List of close-quote characters.
This only has effect if `electric-sentence-end-after-quote' is
non-nil."
  :type '(repeat character))

(defcustom electric-sentence-3rd-space-warning
...
nil for no warning message."
  :type '(string))

(defcustom electric-sentence-3rd-space-inhibit t
...
the limitation by typing \\[quoted-insert] \`SPC' just once."
  :type '(boolean))

(defcustom electric-sentence-enders '(?? ?! ?.)
  "List of characters we consider as ending a sentence."
  :type '(repeat character))
...
(defcustom electric-sentence-max-abbrev-length 10
...
overestimate than to underestimate this."
  :type '(number))

(defcustom electric-sentence-prog-modes '(prog-mode)
...
(defcustom electric-sentence-treat-org-src-as-prog t
...
and strings), even though HTML is not derived from `prog-mode'."
  :type '(boolean))
